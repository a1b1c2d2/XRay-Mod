...
public class ClientTick implements Runnable
{
	...
	private void blockFinder() {
		HashMap<String, Integer> ores = XrayController.blockStore.getDrawableBlocks();
		if ( ores.isEmpty() )
					...
					for ( int i = lowBoundX; i <= highBoundX; i++ ) {
						for ( int j = lowBoundY; j <= highBoundY; j++ ) {
							for ( int k = lowBoundZ; k <= highBoundZ; k++ ) {
								...
								BlockData data = XrayController.blockStore.getStoredBlocks().get( foundKey );
								temp.add( new BlockInfo(x + i, y + j, z + k, data.getColor()) ); // Add this block to the temp list using world coordinates
							}
						}
					}
				}
